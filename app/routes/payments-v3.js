...
router.all('/payments/v3/research-set-up/PoB-clear-data', function (req, res) {
  ...
  router.post('/payments/v3/research-set-up/benefits-answer', function (req, res) {

    var researchSetUpBenefits = req.session.data['researchSetUpBenefits']


  if (!researchSetUpBenefits || researchSetUpBenefits.length === 0) {
    return res.redirect('/payments/v3/research-set-up/ready');
  }
  ...
  res.redirect('/payments/v3/research-set-up/ready');
...
router.post('/payments/v3/how-did-you-find-out-about-this-service-answer', function (req, res) {
  ...
  if (howDidYouFindOutAboutThisService === "heard-about-on-phone" || howDidYouFindOutAboutThisService === "job-centre")  {
    res.redirect('/payments/v3/go-to-identity-check');

  } else {
    res.redirect('/payments/v3/cannot-get-a-proof-of-benefit-letter');
  }
})

// Select which benefit selection page to show based on number of benefits available for user.
router.all('/payments/v3/list-benefits-answer', function (req, res) {

  var researchSetUpBenefits = req.session.data['researchSetUpBenefits']

  if (researchSetUpBenefits == [] || researchSetUpBenefits == undefined)  {
    res.redirect('/payments/v3/you-cannot-use-this-service-no-benefits');
    return

  } else if ((researchSetUpBenefits.length > 0) && (inScope.length > 0 ))   {
      res.redirect('/payments/v3/select-benefits-you-need-proof-of');

  } else if (inScope.length === 0 )  {
    res.redirect('/payments/v3/you-cannot-get-proof-of-benefit-letter');

  }

})

router.post('/payments/v3/select-benefits-answer', function (req, res) {

  var doYouWantLetterFor = req.session.data['doYouWantLetterFor']

    if (doYouWantLetterFor === 'no') {
      res.redirect('/payments/v3/contact-us-for-different-benefit-letter');
    }

    else {
      res.redirect('/payments/v3/where-we-send-your-letter');
    }

})

router.post('/payments/v3/select-benefits-answer-change-link', function (req, res) {
  ...
  var doYouWantLetterFor = req.session.data['doYouWantLetterFor']
    ...
    if (doYouWantLetterFor === 'no') {
      ...
      res.redirect('/payments/v3/check-your-answers');
    }
...
router.post('/payments/v3/send-letter-to-address-answer', function (req, res) {
  ...
  if (confirmLetterSend == "No")  {
    ...
    res.redirect('/payments/v3/you-cannot-use-this-service-incorrect-address');
  }

  else {
    // Send user to check answers
    res.redirect('/payments/v3/digital-or-letter');
  }
...
router.post('/payments/v3/change-link', function (req, res) {
...
router.post('/payments/v3/check-your-answers-answer', function (req, res) {

  var digitalOrLetter = req.session.data['digitalOrLetter']

  if (digitalOrLetter === 'Posted letter') {
    res.redirect('/payments/v3/request-complete');
