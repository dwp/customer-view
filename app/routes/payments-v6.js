...
router.all('/payments/v6/research-set-up/PoB-clear-data', function (req, res) {
  ...
  uCBenefit = false;
  carersBenefit = false
req.session.data.carersBenefit = false
  ...
  router.post('/payments/v6/research-set-up/benefits-answer', function (req, res) {
...
if (!researchSetUpBenefits || researchSetUpBenefits.length === 0) {
    return res.redirect('/payments/v6/research-set-up/ready');
}
    ...
    for (var i=0; i < researchSetUpBenefits.length; i++) {
      ...
      if (researchSetUpBenefits[i] === 'Bereavement Benefit' ||
       ...
       else if (researchSetUpBenefits[i] === 'Carer’s Allowance') {
              carersBenefit = true;
      }
    ...
    req.session.data.carersBenefit = carersBenefit;
  ...
  res.redirect('/payments/v6/research-set-up/ready');
...
router.post('/payments/v6/how-did-you-find-out-about-this-service-answer', function (req, res) {
  ...
  if (howDidYouFindOutAboutThisService === "heard-about-on-phone" || howDidYouFindOutAboutThisService === "job-centre")  {
    res.redirect('/payments/v6/go-to-identity-check');

  } else {
    res.redirect('/payments/v6/cannot-get-a-proof-of-benefit-letter');
  }
})

// Select which benefit selection page to show based on number of benefits available for user.
router.all('/payments/v6/list-benefits-answer', function (req, res) {

  var researchSetUpBenefits = req.session.data['researchSetUpBenefits']

  if (researchSetUpBenefits == [] || researchSetUpBenefits == undefined)  {
    res.redirect('/payments/v6/you-cannot-use-this-service-no-benefits');
    return

  } else if ((researchSetUpBenefits.length > 0) && (inScope.length > 0 ))   {
      res.redirect('/payments/v6/select-benefits-you-need-proof-of');

  } else if (inScope.length === 0 )  {
    res.redirect('/payments/v6/you-cannot-get-proof-of-benefit-letter');

  }

})

router.post('/payments/v6/select-benefits-answer', function (req, res) {

  var doYouWantLetterFor = req.session.data['doYouWantLetterFor']

    if (doYouWantLetterFor === 'no') {
      res.redirect('/payments/v6/contact-us-for-different-benefit-letter');
    }

    else {
      res.redirect('/payments/v6/where-we-send-your-letter');
    }

})

router.post('/payments/v6/select-benefits-answer-change-link', function (req, res) {
  ...
  var doYouWantLetterFor = req.session.data['doYouWantLetterFor']
    ...
    if (doYouWantLetterFor === 'no') {
      ...
      res.redirect('/payments/v6/check-your-answers');
    }
...
router.post('/payments/v6/send-letter-to-address-answer', function (req, res) {
  ...
  if (confirmLetterSend == "No")  {
    ...
    res.redirect('/payments/v6/you-cannot-use-this-service-incorrect-address');
  }

  else {
    // Send user to check answers
    res.redirect('/payments/v6/digital-or-letter');
  }
...
router.post('/payments/v6/change-link', function (req, res) {
...
router.post('/payments/v6/check-your-answers-answer', function (req, res) {

  var digitalOrLetter = req.session.data['digitalOrLetter']

  if (digitalOrLetter === 'Posted letter') {
    res.redirect('/payments/v6/request-complete');
...
router.post('/payments/v6/apply-filters', function (req, res) {

  console.log('ROUTE HIT');
  console.log(req.body);

  const benefit = req.body.type;

  // Handles ESA selected by itself or alongside another benefit
  const hasBenefit =
    benefit === '2' ||
    (Array.isArray(benefit) && benefit.includes('2'));
  ...
  // Date range and ESA
  if (hasDateRange && hasBenefit) {
    return res.redirect(
      '/payments/v6/payment-history-date-and-benefit-filter'
    );
  }
  ...
  if (hasBenefit) {
    return res.redirect(
      '/payments/v6/payment-history-benefit-filter'
    );
  }
  ...
  if (hasDateRange) {
    return res.redirect(
      '/payments/v6/payment-history-date-filter'
    );
  }

  // Nothing selected
  return res.redirect('/payments/v6/payment-history');
